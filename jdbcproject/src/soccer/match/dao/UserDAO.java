...
public class UserDAO {
	...
	String sqlInsert = "INSERT INTO USER_INFO(USER_ID, USER_PW, USER_NAME) VALUES(?, ?, ?)";
	String sqlLogin = "SELECT USER_ID, USER_PW, USER_NAME FROM USER_INFO WHERE USER_ID = ? AND USER_PW = ?";
	...
	String sqlSelectMatch = "SELECT * FROM MATCH_LIST WHERE MATCH_CLOSE = 0";
	...
	String sqlApplyMatch = """
			...
			""";
	String sqlSelectMyMatch = "SELECT * FROM MATCH_INFO WHERE USER_ID = ?";
...
//				resultDTO.setUser_money(rs.getInt("USER_MONEY"));
	...
	public int applyMatch(int matchNo, UserDTO resultDTO, MatchListDTO matchListDTO) {
		...
		try {
			...
			st.setString(2, matchListDTO.getMatch_date());
			st.setString(3, matchListDTO.getMatch_place());
