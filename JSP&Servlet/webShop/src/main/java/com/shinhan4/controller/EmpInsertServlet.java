...
public class EmpInsertServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("/empinsert.do GET 요청");
		...
		System.out.println("직책 건수 : " + jobList.size());
		System.out.println("부서 건수 : " + deptList.size());
		System.out.println("매니저 건수 : " + empList.size());
		
		request.setAttribute("jobList", jobList);
		request.setAttribute("deptList", deptList);
		request.setAttribute("empList", empList);
		
		// 위임(받은 요청을 webapp/jsp/emp/empInsert.jsp로 전달)
		request.getRequestDispatcher("jsp/emp/empInsert.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 직원 등록 후 직원 목록을 보여주기 위해 sendRedirect; /empinsert.do 실행 후 ==> jsp/emp/emplist.do
